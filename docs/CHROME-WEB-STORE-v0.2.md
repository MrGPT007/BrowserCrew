...
B08 remains externally blocked until the reviewed assets are uploaded in the Developer Dashboard and the submission/review requirements are completed. All three required v0.2 visual assets now have exact-candidate engineering proof.

- 128×128 PNG extension/store icon — **ready and packaged** at `src/assets/icons/browsercrew-128.png`; the manifest maps the reviewed 16/32/48/128 icon set and the deterministic release packager requires all four files inside the extension ZIP. CI verifies the 128×128 file against reviewed SHA-256 `8bcec8327cb641996bddf9a0f5817c7e5014c3dd272ab74a25d40491d9023eb7` and copies it into `store-media-evidence`.
...
- 440×280 small promotional tile — **generated by exact-candidate CI** at `artifacts/store-media/browsercrew-small-promo-440x280.png`; `scripts/store-assets-smoke.mjs` renders the existing BrowserCrew `B` mark, product lockup, Grandma-proof control copy, and NeoBrutal Soft palette, then verifies the exact PNG dimensions and records its SHA-256 in the exact-candidate receipt.
- 1400×560 marquee image — optional.
- Product video — optional unless required by the Dashboard flow chosen at submission time.

The CI workflow uploads the screenshot, reviewed 128×128 icon, generated 440×280 promotional tile, and sanitized `report.json` together as `store-media-evidence`, tied to the exact candidate SHA. Do not create media that implies capabilities, providers, permissions, or automatic actions that the shipped extension does not provide.
...
- [x] `npm run store-media-check` is mandatory and verifies executable media-generation/asset contracts.
- [x] Exact-candidate CI generates and validates the required 1280×800 product screenshot.
- [x] Reviewed 128×128 extension/store icon is packaged and machine-verified.
- [x] Exact-candidate CI generates and validates the required 440×280 small promotional tile.
