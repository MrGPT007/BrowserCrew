...
The current repository does not yet contain the required final store media, so B08 remains externally blocked.

- 128×128 PNG extension/store icon in the extension ZIP — **required, missing**.
- At least one product screenshot, 1280×800 preferred (640×400 also supported) — **required, missing**.
...
Use real BrowserCrew UI for screenshots. Do not create screenshots that imply capabilities, providers, permissions, or automatic actions that the shipped extension does not provide.

## Submission checklist

Repository/engineering checks:

- [ ] `manifest.json` and `package.json` identify the same v0.2 package version.
- [ ] `npm run store-readiness-check` passes on the exact release candidate.
- [ ] `npm run check` passes.
- [ ] Full current-stable installed-extension matrix passes.
- [ ] Pinned Chrome 152 matrix passes.
- [ ] Independent 75-attempt release matrix passes.
