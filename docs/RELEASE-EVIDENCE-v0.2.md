...
| Provider support | **Blocked** | Deterministic OpenAI-compatible test servers exercise existing adapters, and the V02-B04 candidate adds native Anthropic Messages-protocol contract plus installed-Chromium coverage without exposing provider raw errors or private thinking | Certify every provider advertised for v0.2 against live external deployments. Deterministic protocol fixtures do not certify live OpenAI, LM Studio, Ollama, or Anthropic services |
...
`V02-B04` — The Anthropic adapter implementation is present in the current candidate: it translates the normalized BrowserCrew request/tool contract to the native Anthropic Messages API, preserves real AbortController cancellation, normalizes generation/streaming/tool calls/usage, keeps API credentials session-only, sanitizes provider errors, and does not expose private thinking. Deterministic adapter and installed-Chromium suites are mandatory on the exact PR head before this engineering blocker can be closed. This does **not** resolve V02-B03 live-provider certification.
