...
| Accessibility | **Blocked** | Semantic controls, visible focus, reduced motion, narrow sidebar styling, and non-color status exist. V02-B05 adds an automated keyboard/Chromium-AX engineering audit plus a manual AT checklist. | Pass the automated exact-head accessibility suite, then perform and record the required human keyboard + screen-reader review of all primary flows; automated AX-tree evidence is not manual NVDA/VoiceOver/JAWS signoff |
...
`V02-B05` — Automated keyboard and Chromium accessibility-tree regression coverage plus the manual release checklist are being added under issue #36. Human keyboard/screen-reader release review has not yet been recorded, so this blocker remains open even if the automated suite is green.
