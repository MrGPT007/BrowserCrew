...
try {
  ...
  assert.ok(providerServer.requests.length >= 2, "Tool flow should make a tool-decision request and a final model request.");
  ...
  const fixtureState = await target.evaluate(() => window.__browserCrewToolFixture);
  assert.equal(fixtureState?.reads, 1, "Exactly one page read should execute for the valid tool call.");
  ...
  for (const phrase of ["asked to read", "Allowed for this message only", "Page read finished", "exact approved tab"]) {
    assert.match(activityText, new RegExp(phrase, "i"));
  }
  ...
  await waitForConversationChange(panel);
  await enablePageRead(panel, target);
  const readsBeforeBadArgs = await target.evaluate(() => window.__browserCrewToolFixture.reads);
  ...
  const readsAfterBadArgs = await target.evaluate(() => window.__browserCrewToolFixture.reads);
  assert.equal(readsAfterBadArgs, readsBeforeBadArgs, "A tool call with model-supplied URL arguments must not execute.");
  pass("BrowserCrew refused model arguments that attempted to widen page.read scope");

  await panel.locator("#chatNewButton").click();
  await waitForConversationChange(panel);
  await enablePageRead(panel, target);
  const readsBeforeStop = await target.evaluate(() => window.__browserCrewToolFixture.reads);
  ...
  const readsAfterStop = await target.evaluate(() => window.__browserCrewToolFixture.reads);
  assert.equal(readsAfterStop, readsBeforeStop, "Stop must prevent a delayed model tool request from dispatching page.read.");
...
async function waitForConversationChange(panel) {
...
async function startFixtureServer() {
  const server = createServer(async (request, response) => {
    ...
    if (url.pathname !== "/chat-tool-page.html") {
...
async function startProviderServer() {
  ...
  const server = createServer(async (request, response) => {
    ...
    const record = { body, stream: body.stream === true, userText: String(userText), hasToolResult: Boolean(toolMessage), toolText: String(toolMessage?.content || "") };
    ...
    if (toolMessage) {
      sendSseContent(response, body.model, "I read the approved page: the warehouse ships on Tuesdays and Fridays.");
      return;
    }
    ...
    if (/unsafe arguments/i.test(userText)) {
      sendToolCall(response, body.model, JSON.stringify({ url: "https://example.invalid/other" }));
      return;
    }
