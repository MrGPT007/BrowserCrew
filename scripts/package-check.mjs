...
for (const file of ["package-lock.json", "scripts/package-release.mjs", "scripts/package-smoke.mjs"]) await access(file);
await Promise.all([
...
if (!workflow.includes("dist")) throw new Error("Quality CI must upload package receipts and the candidate artifact.");

const evidence = await readFile("docs/RELEASE-EVIDENCE-v0.2.md", "utf8");
if (!evidence.includes("| Package integrity | **Partial**")) throw new Error("Package integrity must remain Partial until exact-head package evidence is green.");
if (!evidence.includes("V02-B07")) throw new Error("V02-B07 must remain tracked until exact-head package proof is green and the release ledger is reconciled.");
